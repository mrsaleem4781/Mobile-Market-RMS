...
import { AppUser, UserRole, Market, ReportedMobile, PublicNotice } from '../../types';
...
export default function AuthModule({ onLoginSuccess }: AuthModuleProps) {
  ...
  const [publicReports, setPublicReports] = useState<ReportedMobile[]>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const notices = await db.publicNotices
        .where('status')
        .equals('PUBLISHED')
        .reverse()
        .sortBy('createdAt');
      setPublicNotices(notices.slice(0, 3));

      const reports = await db.reportedMobiles
        .orderBy('reportedAt')
        .reverse()
        .limit(3)
        .toArray();
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setTimeout(async () => {
      try {
        ...
        const newShopId = `shp-${Math.random().toString(36).substr(2, 9)}`;
        ...
        triggerAlert(
          'PENDING',
          "Shop Registration Recorded",
          `Welcome ${regName}! Your shopkeeper profile was created successfully. Your account is pending approval by the market president/admin.`
        );
      ...
      <div className="w-full max-w-5xl bg-slate-900/60 backdrop-blur-xl rounded-3xl border border-slate-800/80 shadow-2xl overflow-hidden grid grid-cols-1 lg:grid-cols-12 min-h-[640px]" id="auth-main-panel">
        ...
        <div className="lg:col-span-5 bg-gradient-to-b from-slate-900 via-slate-950 to-slate-900 p-8 sm:p-10 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-slate-800">
          <div className="space-y-6">
            ...
            {/* Old compliance description hidden in favor of the public notice board. */}
            ...
            {/* Public information board for merchants and market members */}
            <div className="space-y-4 pt-4">
              <div className="space-y-2">
                <span className="text-[10px] text-sky-400 font-extrabold uppercase tracking-widest font-mono">SHUBA ITLAAT / PUBLIC UPDATES</span>
                <h3 className="text-2xl font-black text-white tracking-tight leading-tight">
                  Market Notices and Recovery Reports
                </h3>
                <p className="text-xs text-slate-400 leading-relaxed">
                  President aur authorized team ki taraf se latest reports, recovered phones aur zaroori market itlaat yahan show hongi, taake har shopkeeper ko maloom rahe ke kaam actively ho raha hai.
                </p>
              </div>
              ...
              <div className="space-y-2.5">
                {publicNotices.length > 0 ? (
                  publicNotices.map((notice) => (
                    <div key={notice.id} className="bg-slate-900/90 border border-slate-800 rounded-2xl p-3.5">
                      <div className="flex items-start gap-3">
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 border ${
                          notice.category === 'RECOVERY'
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span className="text-[9px] px-2 py-0.5 rounded-full font-black uppercase tracking-wider bg-white/5 text-slate-300">
                              {notice.isPinned ? 'PINNED' : notice.category}
                            </span>
                          </div>
                          ...
                          <p className="text-[10px] text-slate-500 font-mono mt-2">By {notice.createdByName}</p>
                        </div>
                      </div>
                    </div>
                  ))
                  ...
                  publicReports.map((report) => (
                    <div key={report.id} className="bg-slate-900/90 border border-slate-800 rounded-2xl p-3.5">
                      <div className="flex items-start gap-3">
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 border ${
                          report.status === 'RECOVERED'
                            ? 'bg-emerald-500/10 border-emerald-500/20'
                            : report.status === 'STOLEN'
                              ? 'bg-amber-500/10 border-amber-500/20'
                              : 'bg-rose-500/10 border-rose-500/20'
                        }`}>
                          <Megaphone className={`w-4.5 h-4.5 ${
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[11px] text-slate-400 leading-snug mt-1">
                            Report by {report.reportedByName}. IMEI ending {report.imei1.slice(-4)} is updated in the public alert board.
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 text-xs text-slate-400 leading-relaxed">
                    Abhi koi public update available nahi. President/admin jab report add karega to latest itlaat yahan show hogi.
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 flex gap-3.5 items-center">
              ...
              <p className="text-[11px] text-slate-300 leading-snug font-sans">
                Public notice board live hai. Important recovery, stolen/snatched aur market safety updates yahan regularly publish hongi.
              </p>
            </div>
          </div>
              ...
              <div>
                {!isRegistering ? (
                  ...
                  <div className="space-y-6" id="login-flow-container">
                    ...
                    {/* Guided access shortcuts for local operators */}
                    ...
                    <div className="flex items-center gap-2 text-[11px] text-slate-400 leading-normal border-t border-slate-100 pt-3">
                      ...
                      <p>
                        New shop registrations must be approved by the market president/admin before gaining access.
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <select
                          value="SHOPKEEPER"
                          disabled
                          className="w-full bg-emerald-50 border border-emerald-200 rounded-xl px-3 py-2 text-xs text-emerald-800 font-bold focus:outline-none cursor-not-allowed"
                        >
