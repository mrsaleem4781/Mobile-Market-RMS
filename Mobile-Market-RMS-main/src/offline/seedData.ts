...
import { Market, Shop, Transaction, AppUser, AuditLog, ReportedMobile, PublicNotice } from '../types';
...
export const SEED_PUBLIC_NOTICES: PublicNotice[] = [
  {
    id: 'notice-001',
    title: 'Quaidabad market verification drive started',
    message: 'President Zia Khan Mehsood ki team shop records aur IMEI verification ko update kar rahi hai. Shopkeepers apni buy/sell entry daily complete rakhen.',
    category: 'GENERAL',
    createdAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(),
    createdBy: 'usr-mktadmin-quaid',
    createdByName: 'Zia Khan Mehsood',
    isPinned: true,
    status: 'PUBLISHED'
  },
  {
    id: 'notice-002',
    title: 'Recovered phone handover update',
    message: 'Recovered mobile reports ab public board par show hongi taake market members ko clear progress nazar aaye.',
    category: 'RECOVERY',
    createdAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(),
    createdBy: 'usr-superadmin',
    createdByName: 'Super Admin Bureau',
    status: 'PUBLISHED'
  }
];
...
export async function bootstrapSeedData(force = false) {
  try {
    ...
    if (marketCount === 0 || force) {
      ...
      await db.publicNotices.clear();
      await db.publicNotices.bulkPut(SEED_PUBLIC_NOTICES);
