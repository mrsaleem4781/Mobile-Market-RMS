...
import { Transaction, Shop, Market, AuditLog, AppUser, PublicNotice } from '../../types';
...
export default function SuperAdminDashboard({ currentUser, activeTab }: SuperAdminDashboardProps) {
  ...
  const [noticeCategory, setNoticeCategory] = useState<PublicNotice['category']>('GENERAL');
  ...
  const fetchGlobalDatabase = async () => {
    ...
    const notices = await db.publicNotices.reverse().sortBy('createdAt');
    setPublicNotices(notices);
  ...
  const handlePublishNotice = async (e: React.FormEvent) => {
    ...
    if (!noticeTitle.trim() || !noticeMessage.trim()) {
      setDashboardMessage("Please write a notice title and message before publishing.");
      return;
    }
    ...
    await db.publicNotices.put(newNotice);
    setNoticeTitle('');
    setNoticeMessage('');
    setNoticeCategory('GENERAL');
    setNoticePinned(false);
    setDashboardMessage("Public notice published. It will now appear on the login Shuba Itlaat board.");
    await fetchGlobalDatabase();
  };

  const handleDeleteNotice = async (noticeId: string) => {
    if (!window.confirm("Delete this public notice from the login board?")) return;
    await db.publicNotices.delete(noticeId);
    setDashboardMessage("Public notice removed from the board.");
    await fetchGlobalDatabase();
  };
  ...
  return (
    <div className="space-y-6" id="super-overview-scope">
      ...
      {/* 3. Public notice publisher */}
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4" id="public-notice-publisher">
        <form onSubmit={handlePublishNotice} className="xl:col-span-2 bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-tight flex items-center gap-2 font-mono">
                <Megaphone className="w-5 h-5 text-blue-600" /> Shuba Itlaat Publisher
              </h3>
              <p className="text-xs text-slate-500 mt-1">
                Publish market updates for shopkeepers. Latest notices show on the public login page.
              </p>
            </div>
            <span className="hidden sm:inline-flex text-[10px] font-black uppercase tracking-wider text-blue-700 bg-blue-50 border border-blue-100 rounded-full px-3 py-1">
              Public Board
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="md:col-span-2 space-y-1">
              <label className="text-[10px] text-slate-500 font-bold uppercase tracking-wider font-mono">Notice Title</label>
              <input
                value={noticeTitle}
                onChange={(e) => setNoticeTitle(e.target.value)}
                placeholder="e.g. Recovery report updated"
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={(e) => setNoticeCategory(e.target.value as PublicNotice['category'])}
                className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm font-bold text-slate-700 outline-none focus:border-blue-400 focus:bg-white"
              >
          ...
          <div className="space-y-1">
            <label className="text-[10px] text-slate-500 font-bold uppercase tracking-wider font-mono">Public Message</label>
            <textarea
              value={noticeMessage}
              onChange={(e) => setNoticeMessage(e.target.value)}
              placeholder="Short update in English or Roman Urdu for shopkeepers..."
              rows={4}
              className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm font-medium text-slate-800 outline-none focus:border-blue-400 focus:bg-white resize-none"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-1">
            <label className="flex items-center gap-2 text-xs font-bold text-slate-600 cursor-pointer">
              ...
              />
              Pin this update on the login board
            </label>
            <button
              type="submit"
              className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-xl text-xs font-mono font-black shadow-xs cursor-pointer flex items-center justify-center gap-1.5 uppercase duration-150"
            >
        ...
        <div className="bg-slate-950 text-white rounded-2xl p-5 shadow-xs border border-slate-800 space-y-4">
          <div className="flex items-center justify-between gap-3">
            <h3 className="text-xs font-black uppercase tracking-wider font-mono">Latest Public Notices</h3>
            <span className="text-[10px] text-slate-400 font-mono">{publicNotices.length} total</span>
          </div>
          {publicNotices.length === 0 ? (
            <p className="text-xs text-slate-400 leading-relaxed">No notice published yet. Add the first Shuba Itlaat update from the form.</p>
          ) : (
            <div className="space-y-2.5 max-h-[320px] overflow-y-auto pr-1">
              {publicNotices.slice(0, 5).map((notice) => (
                <div key={notice.id} className="bg-white/5 border border-white/10 rounded-xl p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        {notice.isPinned && <Pin className="w-3 h-3 text-amber-300 shrink-0" />}
                        <p className="text-sm font-extrabold leading-tight truncate">{notice.title}</p>
                      </div>
                      <p className="text-[11px] text-slate-400 leading-snug mt-1 line-clamp-2">{notice.message}</p>
                      <p className="text-[9px] text-slate-500 font-mono mt-2">
                        {notice.category} / {new Date(notice.createdAt).toLocaleDateString('en-GB')}
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => handleDeleteNotice(notice.id)}
                      className="text-slate-500 hover:text-rose-300 p-1 rounded-lg hover:bg-white/5"
                      title="Delete notice"
                    >
